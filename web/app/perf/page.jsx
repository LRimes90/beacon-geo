...
export default function Perf() {
  ...
  async function run(e) {
    ...
    try {
      const r = await fetch('/api/perf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url, strategy }) });
    ...
    <>
      ...
      <main className="wrap">
        <div className="hero">
          ...
          <h1>Quanto è <span className="hl">veloce</span> il tuo sito?</h1>
          <p className="lede">Punteggio Lighthouse reale e Core Web Vitals via PageSpeed Insights di Google — gli stessi numeri che usano i motori di ricerca.</p>
        </div>
        ...
        <form onSubmit={run}>
          ...
          <button type="submit" className="go" disabled={loading || !url}>{loading ? 'Misuro…' : 'Misura →'}</button>
        </form>
        <label className="opt">
          <input type="checkbox" checked={strategy === 'desktop'} onChange={(e) => setStrategy(e.target.checked ? 'desktop' : 'mobile')} /> Analizza come desktop (default: mobile)
        </label>

        {err && <p className="err">⚠ {err}</p>}

        {res && !res.ok && (
          <div className="notice" style={{ marginTop: 18 }}><span>Misurazione non riuscita: {res.reason}</span></div>
        )}
          ...
          <section className="result">
            <div className="rhead">
              ...
              <div className="rmeta">
                ...
                <div className="verdict">{verdict(p.score)}</div>
                <div className="sub">Lighthouse · strategia {res.strategy}{p.field ? ` · dati reali utenti: ${p.field.category}` : ''}</div>
              </div>
            </div>

            <div className="rights" style={{ marginTop: 22 }}>
              <div className="rt">Core Web Vitals <span className="info">— misurati in laboratorio</span></div>
              <ul>
                {p.metrics.map((m, i) => (
            ...
            {p.opportunities.length > 0 && (
              <div className="fixes">
                <h3>Dove guadagnare velocità</h3>
